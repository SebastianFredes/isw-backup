...
if (isset($_POST['id_equipo']) && is_numeric($_POST['id_equipo']) && !empty($_POST['id_equipo'])){   
    // Comprobar que id_falt esta definida && si es un numero && no esta vacia 
    
    include "../clases/conexion.php";
    $id_bd=$_POST['id_equipo'];
    $estado=1;
    ...
    $count_idbdequipo = "SELECT count(id_equipo) as contador FROM equipo eq, estado_revision es
                         ...
                         AND es.nombre_estado = 'devolucion'";
    ...
    $res = mysqli_fetch_assoc($consulta);
        ...
        header('location:../vistas/devolucion.php?error=bdequipo_noexiste');return;
        }else{
            ...
            if(mysqli_query($conexion, $sql)){
                ...
                mysqli_close($conexion);
                header('location:../vistas/devolucion.php?mensaje=reintegrado');

            }else{

                mysqli_close($conexion);
                header('location:../vistas/devolucion.php?error=errorenvio');return;
    ...
    header('location:../vistas/devolucion.php?error=errorfaltante');return;
